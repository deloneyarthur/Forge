...
| `ranking/` | Composite scorer (weights in `config/ranker.yaml`); greedy diversifier (`min_per_hypothesis` floor, D103); learned-verdict-model feature extraction + honest-era dataset builder (`features.py`/`dataset.py`, D132 F1 — shadow-track, nothing wired into scoring until F3) | §6 | `test_ranking/` |
