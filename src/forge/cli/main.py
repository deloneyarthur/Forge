...
- `forge check`      : validates contracts compat and that the DB schema applies
- `forge enumerate`  : previews grammar-valid candidate configs (demo registry; diagnosis)
- `forge prefilter`  : runs the §5.2 battery against enumerated candidates (diagnosis)
- `forge campaign`   : the weekly, zero-input challenger run — the pipeline since the
                       2026-09-14 cutover (D416); `forge campaign status` reads its records
- `forge prereg`     : the preregistration ledger the freeze-governance hook reads (D392)

The daemon (`forge run --loop`) and its operator commands (feedback, healthcheck, status,
grammar, yield-audit, campaigns, ranker-model, check-activations) were removed with the
daemon era (Batch 5 G1); git history holds them.
"""
