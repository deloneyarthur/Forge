...
@app.command("king")
def cmd_king(  # noqa: PLR0915 — one command: search + dry-run preview + submit branch + artifact
    seed: int = typer.Option(0, "--seed", help="RNG root seed"),
    n_search: int = typer.Option(
        2000,
        "--search",
        "-n",
        min=1,
        help="genomes to score against the oracle (the DSR trial count N)",
    ),
    top_k: int = typer.Option(10, "--top-k", "-k", min=1, help="number of kings to surface"),
    oracle_path: Path | None = typer.Option(
        None,
        "--oracle",
        help="durable-score oracle JSON (default: the latest published meta_king_oracle)",
    ),
    out: Path | None = typer.Option(
        None, "--out", help="write the kings as JSON to this path (default: stdout only)"
    ),
    no_dedup: bool = typer.Option(False, "--no-dedup", help="skip the gated-export dedup pass"),
    ...
    submit: bool = typer.Option(
        False,
        "--submit",
        help="SUBMIT the kings to Crucible's inbox + record them (default: dry-run preview)",
    ),
    inbox: Path | None = typer.Option(
        None,
        "--inbox",
        help="Crucible inbox dir for --submit (else forge.yaml crucible.inbox_path)",
    ),
    forge_db: Path | None = typer.Option(
        None, "--forge-db", help="Forge state DB for --submit (else forge.yaml db_path)"
    ),
    config_path: Path | None = typer.Option(
        None, "--config", help="forge.yaml path (for --inbox/--forge-db fallback under --submit)"
    ),
) -> None:
    """Search the grammar for high durable-score genomes ("kings").

    Without --submit this is a DRY-RUN preview (writes nothing to Crucible).

    Reads Crucible's published durable-score oracle (FORGE meta-king A3), scores
    grammar-valid candidates, and prints the top-K by predicted
    ``cpcv_sharpe_p25``. This is a generation-only preview (Phase 0): it writes
    NOTHING to Crucible's inbox. The submission half (source tag + the mandatory
    DSR trial-laundering guard) is gated on Crucible provenance/DSR coordination.
    """
    import json

    from forge.core.contracts_check import check_contracts_version
    from forge.grammar import load_grammar
    from forge.king import gated_tried_hashes, load_oracle, search_kings
    from forge.persistence.registry_loader import load_registry
    ...
    archive_dir = grammar_path.parent / "grammar_archive"
    grammar = load_grammar(grammar_path, archive_dir=archive_dir)
    # Offline-friendly preview, like `enumerate`: production paths have the real
    # registry export so the demo fallback only ever fires in dev/test.
    registry = load_registry(allow_demo_fallback=True)
    oracle = load_oracle(oracle_path)

    tried: frozenset[str] = frozenset()
    if not no_dedup:
        tried = gated_tried_hashes()
    ...
    typer.echo(
        f"oracle target={oracle.target} published_at={oracle.published_at} "
        f"n_train={oracle.n_train} model_ic={oracle.model_ic} "
        f"features={len(oracle.feature_columns)}"
    )
    typer.echo(
        f"searched N={result.n_searched} unique={result.n_unique} "
        f"deduped={result.n_deduped} (dedup vs {len(tried)} gated hashes) "
        f"select={select_mode}"
    )
    for rank, king in enumerate(result.kings, start=1):
        cfg = king.config
        typer.echo(
            f"[{rank:3d}] score={king.predicted_score:+.4f} "
            f"{cfg.hypothesis}/{cfg.dte_bucket} hash={cfg.config_hash}"
        )
    ...
    if submit:
        ...
        inbox_root = inbox
        forge_db_path = forge_db
        if (inbox_root is None or forge_db_path is None) and config_path is not None:
            cfg_yaml = load_forge_config(config_path)
            inbox_root = inbox_root or cfg_yaml.crucible.inbox_path
            forge_db_path = forge_db_path or cfg_yaml.db_path
        if inbox_root is None or forge_db_path is None:
            typer.echo(
                "error: --submit needs an inbox + forge-db "
                "(pass --inbox/--forge-db, or --config for the forge.yaml fallback)",
                err=True,
            )
            raise typer.Exit(code=2)
        if not per_cell:
            typer.echo(
                "warning: --submit without --per-cell queues the global top-K, which the "
                "oracle argmax makes a correlated monoculture; consider --per-cell 2.",
                err=True,
            )
        ...
        batch = BatchContext(
            batch_id=mint_batch_id(
                ...
                extra_inputs=f"{enum_inputs}|meta_king|n={n_search}|k={top_k}|pc={per_cell}",
            ),
            grammar_version=grammar.grammar_version,
            registry_hash=reg_hash,
        ...
        with db_connection(forge_db_path) as conn:
            sub = submit_kings(
                ...
                search_n_trials=result.n_searched,
            )
        typer.echo(
            f"** SUBMITTED meta_king: submitted={sub.submitted_count} "
            f"skipped_dup={sub.skipped_duplicate_count} failed={sub.failed_count} "
            f"-> {inbox_root} (source=meta_king, search_n_trials={result.n_searched}) **"
        )
    else:
        typer.echo(
            "** DRY RUN — kings are NOT submitted to Crucible (pass --submit to queue them) **"
        )

    if out is not None:
        payload = {
            "oracle_published_at": oracle.published_at,
            "oracle_target": oracle.target,
            "seed": seed,
            "n_searched": result.n_searched,
            "n_unique": result.n_unique,
            "n_deduped": result.n_deduped,
            # The trial count the DSR trial-laundering guard must account for.
            "dsr_trial_count_n": result.n_searched,
            "kings": [
                {
                    "rank": rank,
                    "config_hash": king.config.config_hash,
                    "predicted_score": king.predicted_score,
                    "hypothesis": king.config.hypothesis,
                    "dte_bucket": king.config.dte_bucket,
                    "config": king.config.model_dump(mode="json"),
                }
                for rank, king in enumerate(result.kings, start=1)
            ],
        }
        ...
        typer.echo(f"wrote {len(result.kings)} kings -> {out}")
