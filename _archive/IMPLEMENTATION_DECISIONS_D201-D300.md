# Forge — Implementation Decisions archive: D201–D300

Rotated out of `IMPLEMENTATION_DECISIONS.md` 2026-08-06 (repo-simplification Step A3;
D242 rotation precedent). Entries verbatim, chronological, 2026-06-24 → 2026-07-20.
100 entries. **D236 is a 2026-08-06 BACKFILL** — it was written on the
`v23-trend-grammar` branch and lost in a ledger merge, not never-written as the
rotation first concluded; see the provenance note on the entry itself. Earlier slice:
`_archive/IMPLEMENTATION_DECISIONS_D001-D200.md`.
...
> **BACKFILLED 2026-08-06** (operator: "write the D that's free"). This entry was
> written 2026-07-04/06 on the `v23-trend-grammar` branch (commits `ade3344` /
> `7813595` — D237's own note reserved the number for it), merged toward main at
> `2ec850a`, then silently LOST in the branch's second ledger-conflict re-merge
> (`2f2748f` resolved the conflict to main's side). Restored verbatim from
> `git show 7813595:IMPLEMENTATION_DECISIONS.md`; the v23 slice it records shipped
> to production inside the v22→v24 deploy (D254, 2026-07-07T15:05:50Z).
