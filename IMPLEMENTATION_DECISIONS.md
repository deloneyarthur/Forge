...
**STATUS: P1-2 harness SHIPPED (`392c029`, 11 tests: aggregator + builder + CLI smoke + determinism). Daemon byte-identical; nothing flipped. NEXT: run `forge shadow-null run -n 3000` against the live cache to produce the actual per-family ve/all survival delta, then hold for the D220 resolution (≥07-04) before flipping `848a1f67`/`e1a43ba8` one at a time.**
