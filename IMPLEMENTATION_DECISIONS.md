...
**STATUS: contracts 1.23.0 + Forge fix built, mypy-strict clean, targeted tests green; full uncontended suite + commit (both repos) + restart to ACTIVATE the durable fix is the deploy step (the manual flush already unblocked prod, so no urgency). Post-restart record the deploy timestamp.**
