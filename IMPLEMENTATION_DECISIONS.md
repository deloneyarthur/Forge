...
## D194 — 2026-06-20 — Crucible LM-G* generation-items handoff RE-DERIVED → all four already-settled / dormant / Path-C-gated; no Forge production code warranted. Authored the Crucible writeback. RESEARCH + Crucible coordination — no production-loop change.

**Spec section:** Crucible `docs/handoffs/FORGE_HANDOFF_lm_generation_items.md` (the learned-model review's Forge-side items: LM-G1 phenotypic diversity distance, LM-G2 learned generative search, LM-G5 effective-N in the king DSR charge; LM-S3 a Crucible-side note). The handoff explicitly invited re-derivation ("may be stale") and demanded "validate every change with data — do not ship on assumption." Operator relayed it at max effort; chose **"draft the Crucible writeback."**

**Method.** Re-derived against the live tree + decision log (two parallel Explore sweeps + direct reads of `diversifier.py`, the D186/D174/D190 entries, `generation-model-levers.md` §5). Verified every load-bearing quote/D-number before committing them to a cross-team doc.

**Verdicts.**
- **LM-G1 (swap diversifier distance token-Jaccard → phenotypic return/regime distance) → CLOSE, already investigated ([[D186]]).** Collides with a structural fact: *"Forge has zero return/pairwise data (Crucible scores strategies independently, exports per-strategy scalars only)"* — the return-correlation distance is **absent**, not weak. The regime-exposure proxy was tested directly and is **noise (Spearman −0.024)**; the strongest structural proxy (directional, −0.228 stress) is "too weak and the wrong granularity"; the residual is **per-pair, not per-recipe** (uncapturable by any config-descriptor distance). D186's standing decision: decorrelation → ASSEMBLY, no Forge decorrelation axis. The diversifier (`select_top_n`, live via `rank_batch`) is a novelty/dedup filter (§6.3); its `content_key`-Jaccard saturation is appropriate for that role, not a defect. LM-G1's legitimate kernel — directional/hypothesis VARIETY — is already implemented via the [[D103]]/[[D136]] floors inside `select_top_n`.
- **LM-G2 (enumerate-then-rank → learned generative search, rewarded by assembled-book contribution) → SUBSUMED; residual = Path C.** (1) The proposer is *already* a learned generative sampler ([[D094]] learned weights + [[D182]]/[[D183]] yield-map posteriors), not fixed enumeration. (2) The oracle-guided generative-search half was built, ran live, and was **retired** — the king/A3 was verbatim "search the genome space … queue the top genomes into the unchanged §8.7 gauntlet" ([[D174]]), maxed at oracle **~0.78 « the 1.5 wall**, and was retired in [[D190]]; it corroborates the wall, not a break. (3) The novel kernel — reward by assembled-book contribution — is **infeasible Forge-side** (book contribution is a per-pair/joint quantity; the D186 zero-data fact again; the wf_p25 quality lane [[D193]] + variety floors are Forge's generation-side proxy). The ceiling argument resolves to LM-G2's OWN fallback (LM-Q4): `generation-model-levers.md` §5 — *"within v1's long-premium grammar the edge magnitude is exhausted ([[D152]]/[[D154]]), so no generation model unlocks promotion … the unlock remains grammar expressivity (Path C, parked)"*; D186 alt-(c) pre-rejected the heavy generative model on grammar-ceiling + determinism grounds. The actionable residual is **new primitives (Path C)** — operator-gated, parked.
- **LM-G5 (charge DSR by N_eff, not nominal N) → DORMANT** (king retired, [[D190]]). Nuance for the register: Forge passed nominal N and [[D175]] *deliberately* chose it as conservative (*"deflating by full N over-deflates (honest/safe)"*); N_eff would reduce deflation — the opposite direction. On revival, reconcile "accurate N_eff" vs "safe full-N" rather than mechanically mirror Crucible's `effective_n.py`.
- **LM-S3 (Discounted-Thompson yield map) → AWARENESS + one watch-item.** Crucible-side; the Forge exposure is that the [[D182]]/[[D183]] `--cohort-yield`/`--regime-gate-yield` loaders consume `structural_yield_map_latest.json` and must tolerate any new decayed/posterior keys (else a pull could stall the daemon — loads-before-validation looks healthy). Confirm loader-leniency when Crucible advance-flags the schema bump.

**Unifying doctrine (re-affirmed).** Generation owns QUALITY + directional VARIETY (config-token-proxied: wf_p25 lane [[D193]] + [[D103]]/[[D136]] floors); decorrelation + assembled-book contribution are owned at ASSEMBLY ([[D186]]/[[D172]]). Both LM-G1 and the novel half of LM-G2 ask generation to optimize a behavioral/joint quantity it cannot observe pre-assembly.

**Register correction surfaced to Crucible.** King retirement is **D190** (2026-06-19), not "D174/D175 archive 2026-06-19" as the handoff cites; D174/D175 are the 06-16 build + provenance/DSR answer, D177 (06-17) the submit-half build.

**Determinism / hard rules.** No code touched — research re-derivation + a coordination doc. No `src/`/grammar/feedback/gate change (#1/#3/#6 untouched).

**Files:** `PROMPT_CRUCIBLE_LM_GENERATION_ITEMS_RESPONSE.md` (new — the writeback, operator to relay), `STATUS.md`, this entry. NO `src/forge/` / grammar / feedback change → not a deploy, no production-loop impact. Operator's in-flight `PROMPT_CRUCIBLE_*` / `FORGE_THROTTLE_*` left uncommitted (theirs).

**References:** Crucible `FORGE_HANDOFF_lm_generation_items.md` + `learned_models_review.md` §7; [[D186]] (decorrelation→assembly, zero-data), [[D187]]/[[D188]] (quality→generation), [[D172]] (no single config promotes), [[D174]]/[[D175]]/[[D177]]/[[D190]] (king built→retired), [[D094]]/[[D182]]/[[D183]] (learned sampler), [[D103]]/[[D136]] (variety floors), [[D193]] (wf_p25 lane live), `docs/proposals/generation-model-levers.md` §5, [[meta-king-arm-status]] / [[decorrelation-at-assembly-not-generation]] (memory).

**STATUS: LM-G* handoff re-derived — LM-G1 closed (D186), LM-G2 subsumed→Path-C, LM-G5 dormant, LM-S3 watch-item. Writeback authored (`PROMPT_CRUCIBLE_LM_GENERATION_ITEMS_RESPONSE.md`); operator to relay. No production code warranted by the data.**

---

## D195 — 2026-06-23 — Backup/DR: nightly versioned `forge.db` + `models/` backup (ops-hardening sprint, item 1/5, Tier-1 blocker). New `scripts/backup_forge_db.sh` + `forge-backup` systemd timer (04:00). Same-disk default, destination-parameterized; off-box DR pending an operator-provided target. ADDITIVE OPS — daemon untouched.

**Spec section:** none (operational hardening, not a §-spec feature). Origin: a productionization-gap audit this session (4 parallel reviews — ops, backlog, learned-systems, grammar/mission) ranked **"no backup/DR for `forge.db`"** the #1 blocker. `forge.db` (DuckDB, **4.2 GB**, 209,389 submissions / 83,675 verdicts) + `~/forge_data/models/` are the sole, **non-git** copies of months of irreplaceable learning-loop state, with **zero** prior backup (grep-confirmed). Operator chose the "Ops-hardening sprint" thrust; backup/DR is the unconditional first move.

**Decision.** A nightly oneshot timer snapshots → validates → retains versioned copies.
- **Method = the house convention, hardened.** The live DB holds an intermittent RW lock (read-only opens can fail; `docs/tasks/investigate-live.md`), so `cp` the single DuckDB file between write bursts (as `daily_ranker_eval.sh` already does), then **validate** the copy by opening it read-only and querying `submissions`; a torn mid-write copy fails validation and we retry (≤3×). Only a validated copy is published via atomic same-fs rename (`forge_db_<UTC>.duckdb`); `models/` is tar.gz'd + verified. **Retention prunes ONLY after a new validated backup exists** (default keep 14) — a failed run can never delete the last good backup. Validation runs on the venv python directly (no `forge` import) so a broken deploy can't break the backup.
- **DR scope is honest.** `FORGE_BACKUP_DEST` defaults to `~/forge_data/backups` (**same-disk**). This box has a single NVMe (`/` + the spare 100 G LVM volume both on it; no second physical disk, no configured remote). Same-disk backup covers the common faults — accidental deletion, bad migration, fs/logical corruption — but **NOT physical disk failure**. For true off-box DR, point `FORGE_BACKUP_DEST` at a mounted external/remote target; nothing else changes. **Off-box DR remains an open operator decision** (provide a target); the sprint was not blocked on it.

**Determinism / hard rules.** Ops glue, not `src/` — rules #6/#8 don't apply (reads the DB + wall clock exactly as `daily_ranker_eval.sh` does; `date -u` filenames are outside the loop). Touches no grammar/weights/config/gate/service; cannot change what Forge submits (#1/#3 untouched). The daemon (`forge.service`) is NOT modified or restarted — purely additive, so the D104 stop→suite→restart ritual for the *service* does not apply. Reverting = `systemctl --user disable --now forge-backup.timer`; backups are inert data.

**Verification (not assumed — [[D185]] lesson).** Ran end-to-end on the live 4.2 GB DB: published in ~1.3 s, validated read-only as a real DuckDB with all 8 tables (209,389 submissions / 83,675 verdicts). Installed the timer (symlink pattern, matches `forge-ranker-eval`); `list-timers` shows the 04:00 schedule; triggered `forge-backup.service` via systemd → ran clean (two real backups now in `~/forge_data/backups/`). Integration test (`tests/integration/test_backup_script.py`, 3 cases: create+validate / retention-keeps-newest-N / validation-failure-never-deletes) green. Full suite **1678 passed, 1 failed** — the lone failure (`test_expected_contract_version_matches_installed`) is **pre-existing and unrelated**: installed `crucible_contracts` is **1.20.0** (operator's in-flight `uv.lock` bump) vs pin 1.19.0; the §13.5 runtime check is MAJOR-only so the daemon is unaffected, but 1.20.0 needs deliberate adoption (separate work; see sprint item 5).

**Files:** `scripts/backup_forge_db.sh` (new), `deploy/systemd/forge-backup.{service,timer}` (new), `tests/integration/test_backup_script.py` (new), `docs/MANPAGE.md` + `docs/HOW-TO.md` (script/timer entry + restore runbook + health-check line), `STATUS.md`, this entry. **Installed live:** symlinks in `~/.config/systemd/user/` + timer enabled. **Git commit sequenced with the operator** (their uncommitted `uv.lock`/1.20.0 + `PROMPT_*` docs share the tree — commit only my own paths, per the live-tree-concurrent-work discipline).

**STATUS: backup/DR LIVE + VERIFIED — `forge-backup.timer` nightly 04:00, keep-14, same-disk default (off-box DR pending an operator-provided target). Ops-sprint item 1/5 done. Surfaced: contracts 1.20.0 un-adopted (suite red, operator's uv.lock) + Crucible ~32 h stall (D137 guard holding). Next: §7.3 backpressure (Tier 1+2).**

---

## D196 — 2026-06-23 — §7.3 backpressure restored (throttle-proposal Tier 1+2): `STRANDED_AFTER` 8d→5d + an aggregate in-flight-depth block (`max_inflight`, default-off). Ops-sprint item 2/5. Code lands byte-identical (Tier-2 off; Tier-1 activates on restart); enabling is operator-gated.

**Spec section:** §7.3 (rate limiting). Origin: `FORGE_THROTTLE_BACKPRESSURE_PROPOSAL.md` (operator-relayed; this session's audit ranked it the #2 ops gap). Diagnosis: the §7.3 limiter is mechanically healthy but misses its GOAL ("a shallow, learnable queue") by a wide margin — of 184,589 lifetime submissions only 49,998 (27%) ever got a real Crucible decision; 80,163 (43%) were sentinel-flushed orphans, 54,228 (29%) stuck `submitted`; ~1.7× sustained over-submission. Root causes: (1) the completion-fraction gate is per-batch, structurally can't represent aggregate depth; (2) a permanent zombie batch (>=80% gated + a residue of never-decided stragglers aged out of the export window) pins D046's oldest-batch selector; (3) the 8d flush margin sat ~5d above the real submit→decide p99 (~3.1d, measured n=9,405), holding orphans ~3d longer than needed.

**Decision (two tiers, mirroring the proposal).**
- **Tier 1 — `STRANDED_AFTER` 8d → 5d** (`feedback/consumer.py`). The aged-out flush watermark (`max(decided_at) − STRANDED_AFTER`) retires never-decided orphans 3 days sooner, un-pinning the zombie so the existing oldest-batch throttle exposes genuinely-incomplete recent batches again. 5d keeps ~2d headroom over the measured p99. Auto-TIGHTENING (Forge retires dead rows / waits more → Rule #4 permits); takes effect on the next daemon restart.
- **Tier 2 — aggregate in-flight-depth block** (`submission/rate_limiter._evaluate_inflight_depth`): a THIRD independent block reason beside the completion fraction + the D137 stall guard, mirroring the stall guard's shape exactly. Genuine in-flight depth = `submitted` rows newer than the flush watermark (excludes the dead tail the flush will retire — else the orphan backlog would pin the depth block exactly as it pinned D046). Blocks when depth > `max_inflight`. Knob `submission.max_inflight` (forge.yaml); **default 0 = off → byte-identical** (function default + the D137 determinism pattern). Recommended enable value: 600 (3× batch_size = the §7.3 "one batch at a time" intent).

**Determinism / hard rules.** Rule #3 untouched (no gate change — this throttles Forge, not Crucible's bar). Rule #4: auto-TIGHTENING (Forge submits LESS) → shippable, kept operator-gated per the proposal. Rule #6: Tier-2 default-off is byte-identical (golden contract `test_guard_off_equivalence`); enumeration untouched. The depth metric imports `STRANDED_AFTER` from consumer (drift-proof, single source of truth; no circular import — verified).

**TDD.** Invariants-first (`tests/invariants/test_inflight_depth_invariants.py`, red→green confirmed — the param didn't exist → TypeError): guard-off equivalence, block-iff-over-cap, stranded-tail exclusion, + the proposal's zombie scenario (oldest batch reads ≥80% / pct says "clear" yet the aggregate depth blocks). The D185 multi-call-site trap: `max_inflight` wired to BOTH `_run_one_iteration` call sites; `test_run_loop` asserts the `--loop` path forwards it (a missing kwarg silently takes the default — mypy can't catch it), `test_config_threading` asserts the config→resolved path (0 default / 600 from yaml).

**Verification.** ruff + mypy --strict (90 files) clean; full suite 1682 passed / 1 failed (the lone failure is the pre-existing un-adopted contracts 1.20.0 pin — Finding A, unrelated). Wired but NOT enabled: forge.yaml has no `max_inflight` → default 0 → the daemon is byte-identical until the operator opts in.

**Files:** `src/forge/submission/rate_limiter.py`, `src/forge/feedback/consumer.py`, `src/forge/cli/main.py`, `src/forge/config/forge_config.py`, `tests/invariants/test_inflight_depth_invariants.py` (new), `tests/unit/test_cli/test_config_threading.py`, `tests/unit/test_cli/test_run_loop.py`, `docs/MANPAGE.md`, `docs/HOW-TO.md`, `STATUS.md`, this entry.
...
## D236 — 2026-07-04 — v22→v23 trend-grammar hygiene: add sma_slope + ad_slope, prune the momentum duplicate + weak trend directionals (Crucible signal-quality handoff)

**Spec section:** §3.5 S4 (signal-horizon → DTE derivation) + §5 enumeration policy. Classification: enumeration-policy bump (`docs/tasks/grammar-change.md` #2) — Forge-owned threshold/horizon tables only; the §3.5 `rules:` text is UNTOUCHED (no hard-rule-#1 gate). Source: `../Crucible/docs/handoffs/FORGE_signal_quality_champions_2026-07-03.md` (operator-directed cross-sectional signal-quality + compounding study). Drafted 2026-07-03, committed 07-04.

**The change (trend_continuation directional pool; `enumeration/indicator_thresholds.py` + `grammar/signal_horizon.py`).** §2.1/§2.5 — ADD `sma_slope` (SMA-200 slope; the study's best trend signal, cross-sectional rank-IC **0.078** @63d, DOMINATES `momentum_252`'s 0.059 — holding sma_slope, momentum_252 adds −0.012 incremental) and `ad_slope` (volume champion, registry family=trend, weak IC 0.016) as **percentile-only** directionals. §2.2 — PRUNE `returns_12m_skip1` from the directional pool (cross-sectional rank-corr **1.0** with `momentum_252` — identical 12-1 computation; double-sampling wastes search breadth / inflates effective-N in alpha-budget accounting). §2.3 — PRUNE `macd` / `ema_cross`(12/26) / `supertrend` as trend directionals (anti-momentum at 3-6m or redundant with sma_slope per the IC audit; golden-cross `ema_cross_50_200` is a separate, unregistered id). Pruning = null the directional fields (`is_threshold_skippable('directional')=True`), keeping the entries documented; regime fields left dormant (unused). `momentum_252` RETAINED.

**Rationale + HONEST SCOPE (handoff §0 — this is the load-bearing caveat).** This is a **cross-sectional EQUITY selection-quality** result. It sharpens Forge's trend SELECTION (an A/B shows sma_slope lifts WF median **+2.5%**) but does **NOT** move the single-name options **CPCV-p25 promotion wall** (flat-to-**−3.6%**). sma_slope's IC monetizes in QuantIQ's cross-sectional equity arm, not Forge's single-name long-options path. So this is **grammar HYGIENE, not a promotion lever** — expect no lift in the promotion rate; the value is (a) ranking trend on a strictly-better signal instead of the dominated `momentum_252`, and (b) eff-N/alpha-budget cleanliness from the dedup. The producer's actual promotion lever is unchanged: single-name `volatility_event` supply ([[D216]]/[[D235]]); notably the study's #1 book pool-wide (cpcv-p25 1.51) is itself a single-name vol_event config (`days_to_fomc` + `put_wall_distance_pct`), independently corroborating that thesis.

**Design choices (defensible defaults — flagged for operator review before deploy).** (1) **Percentile-only** ranges `(0.80, 0.90)`, op `>` (the `option_momentum`/D138 precedent): sma_slope/ad_slope are brand-new registry ids with NO Forge-side value distribution and none in `docs/INDICATOR_THRESHOLDS.md`, so an absolute range would be fabricated — percentile is distribution-free (fire when the slope ranks in the top decile of its trailing window = strong uptrend) and needs **no Crucible data dependency**. (2) **Horizons**: sma_slope=200 → long_lookback (the SAME DTE-bucket class as the `momentum_252` it out-ranks → a clean signal swap, not a bucket move); ad_slope=60 → medium_lookback. (3) `momentum_252` **kept, not hard-swapped** — the learned D106 directional-bucket weight ranks sma_slope against it on live evidence (§2.2 only forbids the *duplicate*, not a second distinct 12-1 signal). (4) ad_slope's IC (0.016) is marginal and cuts slightly against the §2.2/§2.3 eff-N goal — included only because §2.5 explicitly asks for its coverage; **operator may prefer to omit it**.

**Deferred / not done.** The `docs/tasks/grammar-change.md` piggyback (add `gamma_flip_distance_pct` + `market_state` to R2 `evidence_to_relax`, D204) was **NOT** folded in — it edits `rules:` text (grammar-doc-sync territory) and this bump is deliberately Python-tables-only. It stays available for the next `rules:`-touching bump.

**Alternatives considered.** Absolute threshold range (rejected — no distribution; would need a Crucible ask, and percentile is arguably the better permanent choice for a slope). Hard-swap momentum_252 → sma_slope by pruning momentum_252 too (rejected — let the learned weight decide; one-line change if the operator wants it). `is_skip=True` for the pruned ids (rejected — semantically means "can't be thresholded"; these are threshold-capable signed indicators, so role-specific nulling is more honest).

**Verification.** Emission proof (seed 0, 4000 cands, live registry): trend_continuation directional mix now `donchian 141 / momentum_252 138 / sma_slope 137 / ad_slope 134 / rolling_sharpe 113 / option_momentum 110`; the four pruned ids emit **zero**; grammar_version reads v23. TDD: `tests/unit/test_enumeration/test_trend_grammar_v23.py` (9 tests, red→green). Suites: grammar + enumeration + invariants **685 green** (no golden re-pin needed — determinism tests verify reproducibility, not a hardcoded sequence, and the change is deterministic, rule #6). Full suite + ruff + mypy --strict clean.

**STATUS: BUILT + committed on branch `v23-trend-grammar` (worktree `../Forge-build-v23`), NOT deployed — daemon on `main`/v22 untouched.** Deploy is operator-gated (`docs/tasks/deploy.md`): stop service → full uncontended suite → ff-merge → restart → verify journal. **Before deploying, MEASURE the CPCV-p25 delta on the resulting trend cohort** (§0 warns −3.6%) so trend hygiene for QuantIQ doesn't silently drag Forge's own tail. On deploy, relay the v22→v23 string + timestamp to Crucible for `crucible funnel --compare v22 v23` (`docs/tasks/crucible-handoff.md`). Non-asks from the handoff also retire two Forge backlog threads: the low-prior E/P fundamental-value pre-check (§3: value/quality null at mega-cap) and any put/bearish arm (§3; already Forge policy, D066).

### D236 addendum — 2026-07-06 — v23 scope EXTENDED per the corrected handoff: chandelier default trend exit + atr_multiplier sweep (§2.7), tighten VE `days_to_fomc` window (§2d). MR asks HELD. Still built-not-deployed.

**Trigger.** The handoff `FORGE_signal_quality_champions_2026-07-03.md` was revised (2026-07-04 + 2026-07-06 edits) with new campaign data. Operator: *"Build v23 with the new hand-off. It's the same changes but with data backing it up now. We should also still hold for MR experiment."* → fold the net-new asks into the still-unshipped v23 bump; hold the MR asks. These are Python-side enumeration changes (exit schema in `custom_predicates.py`, exit params + threshold table in `sampler.py`/`indicator_thresholds.py`) — `grammar.yaml` `rules:`/`grammar_version` UNTOUCHED, so they ride the existing v23 bump (unshipped → determinism-safe; no v24). Version stays v23; deploy story stays `v22→v23`.

**§0 REVERSAL (the headline).** The original §0 caveat above — *"sma_slope lifts WF +2.5% but CPCV-p25 flat-to-−3.6%; hygiene not a lever"* — is **retracted by Crucible**. The −3.6% came from a naive A/B (`sma_slope > 0` grafted onto momentum_252 components). Tested *fairly* — sma_slope as the cross-sectional RANKER, the way it's used — it **WINS the binding CPCV-p25 gate +18% (0.71 vs 0.60)** and leads WF, holding #1 across ~15 trend signals (§2.1a, `ranker_tier2_rerun.json` n=6). **→ the "MEASURE CPCV-p25 first, −3.6% warning" deploy gate above is OBSOLETE** — the concern flipped in v23's favor. The **portfolio-promotion caveat STILL stands** (§0/§4): better component selection/WF/tail, NOT a higher promote rate (the bear tail is structurally un-timeable) — a stream-quality lift, not a promotion unlock ([[promotion-gate-tiers-and-constraint]]).

**§2.7 — chandelier_exit is now the default discretionary trend exit (`custom_predicates.py` `_S5_HYPOTHESIS_EXITS`).** DROPPED `parabolic_sar_exit` from `trend_continuation.required_from_set` → pool = `(trailing_atr, chandelier_exit)`. Crucible's real-backtest exit sweep (14 rules, WF+CPCV n=6) found chandelier beats the parabolic_sar incumbent **+0.29 CPCV-p25 AND higher WF (1.62 vs ~1.33)** — lifts BOTH tail and center (rare) because it lets winners run but cuts reversals where parabolic whipsaws. `trailing_atr` (not refuted) kept. This is a TIGHTENING (fewer options → hard rule #4 OK). **Tuned trail (`sampler.py` `_exit_params`):** chandelier now samples `atr_multiplier` ∈ **[2.0, 3.0]** (Crucible's chandelier template reads it — the D138/D169 template-knob precedent). A tighter trail (≈2.0) adds **+0.155 CPCV-p25** over the 3.0 default (`exit_param_sweep.json`, tail-for-center knob); the range lets the gates weight tail (2.0) vs center (3.0). NOT stacking a loss-cut (premium_stop/breakeven_ratchet add ~0 once the trail is tight) and NOT sampling profit-target/time-stop/regime-flip (refuted/crater).

**§2d — VE `days_to_fomc` event-proximity window tightened (`indicator_thresholds.py`).** `regime_range (7.0, 60.0) → (7.0, 14.0)`. VE books fired at a median ~31d before the event (regime window to 60d) — too wide; the directional dealer-wall / call-wall edge concentrates ≤~10d pre-FOMC and decays wider, while ≤5d loses too many trades. op `<` → fires when `days_to_fomc < threshold`, so [7, 14] centres firing on the ~10d sweet spot (median 10.5d, emission-proof median 10.3d). Handoff calls this "the most actionable threshold change in the doc." FOMC-only — cpi/nfp/opex keep their ranges pending their own evidence (handoff ranks FOMC > opex > cpi/nfp). A tightening (hard rule #4 OK).

**HELD (operator directive "still hold for MR experiment").** All §2b MR asks (prefer `rsi_14`, drop `zscore_returns`, keep `rsi_2` short-horizon) and §2d's MR `rsi_14` [25,30] bullet are NOT in this slice. **CONFIRMATORY / no-op (not built):** §2.6 (`rv_rank` is the king gate — do NOT swap/supplement or sample windows; already Forge policy), §2c (VE = directional dealer-wall + `days_to_fomc`; the archetype Forge already mints), §2.5 (sma_slope/ad_slope coverage — done in the base D236 slice). Engine gotcha logged (§2.1a fix `ed39741`): param-*windowed* xsect rankers need `min_bars_required()` or they silently zero-trade — doesn't bite v23 (sma_slope/ad_slope fixed-window), but a hard prereq for any FUTURE swept-lookback ranker.

**Verification.** TDD `tests/unit/test_enumeration/test_trend_grammar_v23_exits.py` (8 tests, red→green). Golden re-pin (deliberate, ritual step 3): the exit-pool + window changes shift the cold sampler sequence, so `_COHORT_GOLDEN_PRE_REFACTOR` (idx 13-14) and `_REGIME_GOLDEN_PRE` (idx 3+) in `test_sampler.py` were rebaselined — **`none==empty` re-verified for both**, so the D104 flag-OFF byte-identity guarantee holds; only the snapshot moved. Emission proof (seed 0, 4000 cands, live registry): trend exits split `trailing_atr` 390 / `chandelier_exit` 373, **parabolic_sar_exit = 0** in trend; chandelier `atr_multiplier` ∈ [2.00, 3.00] (n=785); `days_to_fomc` regime thresholds ∈ [7.1, 14.0], **median 10.3**. Grammar+enumeration+invariants **693 green**; ruff + `mypy --strict` (101 files) clean. Full suite **1856 green, 2 fail** — both confirmed NOT this change: `test_expected_contract_version_matches_installed` (pre-existing branch staleness — pin 1.22.0 vs installed 1.24.0; fails on pristine `ade3344` too) and `test_held_out_platt_reduces_ece_vs_raw` (flaky — passes in isolation both pristine and with this change; ranking code untouched, no global-RNG use).

**DEPLOY PREREQ FLAGGED (branch contracts staleness).** The v23 branch pins `FORGE_EXPECTED_CONTRACT_VERSION = "1.22.0"` (built 2026-07-03, before main's D244 1.24.0 adoption). Before deploying v23 the operator must adopt 1.24.0 on the branch (pin in `contracts_check.py` + `uv.lock` + fixtures + `forge check`), same ritual as D244 on main — else the CLI startup contract-version check fails. This slice deliberately did NOT bundle a partial (uv.lock-only) contracts bump. **Sequencing:** a grammar bump re-confounds the two in-flight flip preregs (clean-cohort clock reset 2026-07-06T15:54:09Z, D245) → deploy stays queued behind prereg resolution per the D220 one-at-a-time discipline.
