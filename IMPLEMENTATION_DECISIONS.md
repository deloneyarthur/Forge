...
**STATUS: BUILT + VERIFIED (unit/invariants); DEPLOY PENDING the operator gate** — Forge cannot run the full suite uncontended (the live `forge.service` holds the DuckDB write lock) and does not self-restart onto a grammar bump. Deploy ritual (operator): stop service → full uncontended suite → commit D102 + v8 → restart onto v8 → tell the Crucible agent the new version string is `v8` for `crucible funnel --compare v7 v8`. Watch the first post-restart batch: `ranked_top_n_by_hypothesis` / the per-hypothesis bucket mix should show trend_continuation leaving swing_short (the degenerate-S4 fix going live).
