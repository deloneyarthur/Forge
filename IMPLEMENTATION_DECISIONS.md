...
**Pending:** the 6 numbered operator decisions in the proposal's §8 (track shape, solver dependency, slot choice, F3 criterion, floor constants, refit-children rows). On F3 approval the §6.2 generalization becomes the first entry in `docs/DECISIONS.md` (the design-level log, currently empty). Hard-rule compliance table and the 7 RED-first invariant tests are specified in the proposal §5.
