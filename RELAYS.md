...
| `PROMPT_CRUCIBLE_V44_DEPLOYED.md` | → | held (NEW 07-21) | carry (v44 live 03:43:00Z; pins the +2wk read) — **but likely SUPERSEDED by v45**: read pins at the v45 deploy, not v44 (see Q46_READ_INVERSION) | D317 |
| `PROMPT_CRUCIBLE_Q46_READ_INVERSION.md` | → | held (NEW 07-21) | carry (accepts inverted read + hurst-only + ~2× dial; our verdicts reproduce their split; v44-live→v45-refinement, pin read at v45); **gated on operator v45 go** | D318 |
...
| `PROMPT_CRUCIBLE_REFUTATION_REGISTRY_REPLY.md` | → | answered (their `FORGE_refutation_registry_reply_ack_2026-07-21.md`: all 3 corroborations re-derived + reproduce; 3 amendments shipped `c3e90e4`; consumption path SHIPPED — `refutations_*.json` export + `load_refutations_from_export` reader in contracts **1.34.0**) | WIRING still operator-gated (unblocked now); adoption of 1.34.0 staged to ride v44; archive after wiring decision | D313 |
