...
# ---------------------------------------------------------------------------
# H4 (orthogonal-yield) — the (hypothesis, directional, underlying-NAME) factor
# cell discount tilts `_pick_underlying` away from over-mined single names (D108:
# name granularity, so the discount spreads an over-mined name across its minting
# peers rather than toward the non-minting diversified class). None/empty stays
# byte-identical to the D105/D106 underlying draw; the exploration floor survives.
# ---------------------------------------------------------------------------


def _pick_underlyings_with_discount(
    factor_cell_discounts: dict[str, float] | None,
    *,
    class_weights: dict[str, float] | None = None,
    name_weights: dict[str, float] | None = None,
    n: int = 3000,
) -> dict[str, int]:
    """Run `_pick_underlying` n times for volatility_event against the offline
    fallback pool, with an H4 name->discount slice applied."""
    import forge.enumeration.sampler as sampler_mod
    from forge.enumeration.sampler import _load_underlyings, _pick_underlying

    original_dir = sampler_mod._UNIVERSE_EXPORT_DIR
    sampler_mod._UNIVERSE_EXPORT_DIR = Path("/nonexistent_h4_test_dir")
    try:
        _load_underlyings.cache_clear()
        rng = random.Random(0x4)
        counts: dict[str, int] = {}
        for _ in range(n):
            u = _pick_underlying(
                rng,
                "volatility_event",
                (),
                underlying_class_weights=class_weights,
                underlying_name_weights=name_weights,
...
def test_h4_discount_cold_start_byte_identical() -> None:
    ...
    try:
        _load_underlyings.cache_clear()
        class_w = {HIGH_IDIO_VOL: 0.04, DIVERSIFIED: 0.002}
        r0, r1, r2 = random.Random(5), random.Random(5), random.Random(5)
        base = [
            _pick_underlying(r0, "volatility_event", (), underlying_class_weights=class_w)
            for _ in range(60)
        ]
        none_disc = [
            _pick_underlying(
        ...
        assert base == none_disc == empty_disc
    finally:
        sampler_mod._UNIVERSE_EXPORT_DIR = original_dir
        _load_underlyings.cache_clear()
...
def test_h4_discount_tilts_draw_away_from_crowded_name() -> None:
    ...
    class_w = {HIGH_IDIO_VOL: 0.04, DIVERSIFIED: 0.04}  # flat: every name equal at name cold-start
    base = _pick_underlyings_with_discount(None, class_weights=class_w)
    disc = _pick_underlyings_with_discount({"AAPL": 0.2}, class_weights=class_w)  # crush AAPL 5x
    assert disc.get("AAPL", 0) < base.get("AAPL", 0) * 0.5  # AAPL share cut hard
    # mass is redistributed across peers, not lost (total draws preserved)
...
def test_h4_discount_preserves_exploration_floor() -> None:
    ...
    counts = _pick_underlyings_with_discount(
        {"AAPL": 0.001, "NVDA": 0.001},  # crushed ~1000x
        class_weights={HIGH_IDIO_VOL: 0.04, DIVERSIFIED: 0.04},
    )
    assert counts.get("AAPL", 0) > 0  # the floor keeps the crushed names alive
    assert counts.get("NVDA", 0) > 0
...
def test_h4_discount_slices_by_hypothesis_and_directional_in_sample_config(
    ...
    on AAPL for volatility_event cuts AAPL's draw share whichever vol_event
    directional is drawn. Mocks the universe export to the deterministic fallback
    pool (which contains AAPL)."""
    import forge.enumeration.sampler as sampler_mod
    from forge.enumeration.sampler import _load_underlyings
    from forge.enumeration.underlying_class import DIVERSIFIED, HIGH_IDIO_VOL
    ...
    try:
        ...
        def _aapl_share(discounts: dict[tuple[str, str, str], float] | None) -> float:
            aapl = total = 0
            for seed in range(400):
                cfg = sample_config(
                    space,
                    ...
                    underlying_class_weights={HIGH_IDIO_VOL: 0.04, DIVERSIFIED: 0.04},
                    orthogonal_yield_discounts=discounts,
                )
                if cfg.underlying is None:
                    continue
                total += 1
                if cfg.underlying == "AAPL":
                    aapl += 1
            return aapl / total if total else 0.0

        baseline = _aapl_share(None)
        # discount AAPL across EVERY vol_event directional so the slice bites
        # whichever directional this config draws.
        discount_map = {
            ("volatility_event", d, "AAPL"): 0.1
            for d in space.directional_indicators_by_hypothesis["volatility_event"]
        }
        discounted = _aapl_share(discount_map)
        assert baseline > 0  # AAPL is drawn in the baseline
        assert discounted < baseline  # AAPL yields draw share to its peers
    finally:
        sampler_mod._UNIVERSE_EXPORT_DIR = original_dir
        _load_underlyings.cache_clear()
